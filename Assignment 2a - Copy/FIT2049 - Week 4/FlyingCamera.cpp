...
FlyingCamera::FlyingCamera(InputController* input, Vector3 startPos, GameObject* target)
{
	m_input = input;
...
void FlyingCamera::Update(float timestep)
{
	...
	SetPosition(m_objectToFollow->GetPosition());
	// We're going to need this a lot. Store it locally here to save on our function calls 
	Vector3 currentPos = GetPosition() + Vector3(0.0f, 1.5f, 0.0f);
